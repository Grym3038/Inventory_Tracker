  ...
  <div class="w-full flex flex-col overflow-y-auto p-4 bg-white dark:bg-gray-900">
    <div class="w-full flex items-center justify-between">
      <header class="py-4 flex flex-col">
        <h1 class="text-xl font-bold text-gray-900 dark:text-white"><?= empty($item->id) ? 'Add New Product' : 'Edit Product' ?></h1>
        <h5 class="text-sm text-gray-600 dark:text-gray-400"><?= empty($item->id) ? 'Create a new product in your inventory' : 'Update product information' ?></h5>
      </header>
    ...
    <div class="flex flex-col overflow-y-auto h-full">
      <div class="max-w-2xl mx-auto w-full">
        ...
        <form method="POST" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 shadow-xl rounded-2xl p-8">
          ...
          <?php if (isset($item)): ?>
            <div class="mb-6 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 rounded-xl">
              Debug: Item ID = <?= $item->id ?? 'null' ?>
            </div>
          <?php else: ?>
            <div class="mb-6 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-200 rounded-xl">
